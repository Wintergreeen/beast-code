...
DELIMITER //
DROP FUNCTION IF EXISTS RUSH_ALLWD //
CREATE FUNCTION RUSH_ALLWD(
	date_param date,
    team_id_param INT
)
	RETURNS INT
...
DELIMITER //
DROP FUNCTION IF EXISTS PASS_ALLWD //
CREATE FUNCTION PASS_ALLWD(
	date_param date,
    team_id_param INT
)
	RETURNS INT
    ...
    BEGIN
		...
		FROM
			(SELECT
				full_game_stats.t2_rush opp_pass
			FROM full_game_stats
            WHERE 1=1
				AND full_game_stats.t1_id = team_id_param
                AND full_game_stats.game_date < date_param
...
SELECT
    ...
    RUSH_ALLWD(full_game_stats.game_date, full_game_stats.t2_id),
    PASS_ALLWD(full_game_stats.game_date, full_game_stats.t2_id),
    game.location,
